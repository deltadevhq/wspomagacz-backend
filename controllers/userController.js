...
const fetchUserActivity = async (req, res) => {
  try {
    ...
    // Check if provided user_id is equal to logged_user_id
    if (logged_user_id === Number(user_id)) visibility = 'private';

    const activities = await userModel.selectUserActivities(user_id, visibility, offset, limit);
    if (!activities) return res.status(404).json({ error: 'Activities not found.' });
    
    res.status(200).json(activities);
  } catch (error) {
    console.error('Error fetching user activity:', error.stack);
    res.status(500).json({ error: 'Internal server error' });
  }
};
