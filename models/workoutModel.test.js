...
describe('Workout Model', () => {
  describe('Creating workout', () => {
    ...
    test('fails to create a workout with a date that already has a workout', async () => {
      const { related_workout_id, user_id, exercises, date, notes } = await createTestWorkout();

      await expect(createWorkout(related_workout_id, user_id, 'Test workout', exercises, date, notes)).rejects.toThrowError();
    });
  });
