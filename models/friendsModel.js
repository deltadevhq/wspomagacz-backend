...
const selectFriends = async (user_id) => {
  const query = `
    SELECT u.id,
           ...
           u.weights,
           u.height,
           f.modified_at AS friends_since
    FROM friends AS f
...
const updateFriendRequestWithAcceptance = async (sender_id, receiver_id) => {
  const query = `
    ...
    WHERE status = 'pending'
      AND sender_id = $1
      AND receiver_id = $2
    RETURNING *, (SELECT jsonb_build_object('id', id, 'display_name', display_name)
                  FROM users
                  WHERE id = $1) AS sender, (SELECT jsonb_build_object('id', id, 'display_name', display_name)
                                             FROM users
                                             WHERE id = $2) AS receiver
  `;
  const values = [sender_id, receiver_id];
